...
def show_directed_graph(graph, shortest_paths=None, filename="directed_graph.png"):
    ...
    # 使用spring_layout并设置随机种子以获得稳定的布局
...
def calc_shortest_path(graph, word1, word2=None):
    ...
    if word2 is None:
        ...
        for target in graph.nodes():
            if target != word1:
                try:
                    paths = list(nx.all_shortest_paths(graph, source=word1, target=target, weight='weight'))
                    if paths:
        ...
        while True:
            ...
            if choice == "1":
                ...
                if word2.strip() == "":
                    ...
                    # 计算两个单词之间的最短路径
                    shortest_paths = calc_shortest_path(graph, word1, word2)
                    if isinstance(shortest_paths, str):
                        ...
                        for path in shortest_paths:
                            print(f"路径: {' -> '.join(path)}")
                        # 展示图形并高亮显示最短路径
                        show_directed_graph(graph, shortest_paths, "highlighted_graph.png")
